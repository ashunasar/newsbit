...
<!--					<h4 class="p-title mt-30"><b>CRYPTO MINING NEWS</b></h4>-->
            ...
            $count_row = ceil($count_row/10);
             ?>
                    ...
                    $query = "SELECT * FROM `user` ,`post` WHERE post_by=user_username and post_title like :title or post_keyword like  :keyword and post_by=user_username and post_status='Publish' ORDER BY date DESC LIMIT $page_1,10";
                    $result = $conn->prepare($query);
                    $result->execute(['title'=> '%'.$search . '%','keyword'=> '%'.$search . '%']);
